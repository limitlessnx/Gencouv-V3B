...
function intentFor(message: string) {
  ...
  if (/portfolio|managed|management|copy trading|copy trade|pamm|pam trading|pam system|pm service/i.test(message)) return "portfolio";
  return "general";
}
...
function fallbackReply(intent: string) {
  ...
  if (intent === "portfolio") return `Gencouv Portfolio Management is separate from Trading Bots. Clients keep funds in their own supported brokerage account, and participation is subject to eligibility and human onboarding approval. If you are ready to join, the next step is the Gencouv Telegram onboarding agent at ${SUPPORT_TELEGRAM_URL}. Historical master-account performance is available at ${MYFXBOOK_URL}.`;
  return "I can help with Gencouv Trading Bots, product requirements, purchases, My Library access, Portfolio Management, PAM/PAMM and copy-trading onboarding, broker information and risk information.";
}
