...
export async function POST(request: Request) {
  ...
  try {
    ...
    const product=paymentProduct(String(body?.productId||""));
    if (!product) return NextResponse.json({error:"Unknown product."},{status:400});

    const supabase=await createClient();
    const {data:{user}}=await supabase.auth.getUser();
    if (!user) return NextResponse.json({error:"Sign in before purchasing marketplace products."},{status:401});

    const admin=createAdminClient();
    const origin=new URL(request.url).origin;
    const orderId=`GC-${product.sku}-${Date.now()}-${crypto.randomUUID().slice(0,8).toUpperCase()}`;
    const {error:orderError}=await admin.from("marketplace_orders").insert({
      order_id:orderId,user_id:user.id,sku:product.sku,product_slug:product.slug,license_tier:product.licenseTier,
      price_amount:product.price,price_currency:product.currency,payment_status:"creating"
    });
