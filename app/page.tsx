...
const services = [
  { name: "Copy Trading", label: "Primary service", description: "Follow a Gencouv master strategy from your own Lirunex account, with two risk profiles and human-verified onboarding." },
  { name: "Expert Advisors", label: "Trading Bots", description: "Automated trading software and tools for clients who are looking for technology products rather than portfolio management." },
  { name: "Indicators & Utilities", label: "Trading Bots", description: "Decision-support indicators, analysis tools and trading utilities. Marketplace availability is being expanded." },
];
...
export default function Home() {
  return <main>
    ...
    <section className="section shell"><div className="sectionHead"><div><div className="eyebrow left">GENCOUV SERVICES</div><h2>Copy Trading first. Additional tools through Trading Bots.</h2></div><p>Copy Trading is our current primary client journey. Trading Bots separately provides Expert Advisors, indicators and trading utilities.</p></div><div className="serviceGrid">{services.map((service,i)=><a className="service" href={i === 0 ? "/copy-trading" : "/marketplace"} key={service.name}><span>0{i+1}</span><small>{service.label}</small><h3>{service.name}</h3><p>{service.description}</p><b>{i === 0 ? "Explore Copy Trading ↗" : "View Trading Bots ↗"}</b></a>)}</div></section>
    ...
    <footer className="footer shell"><div className="brand"><span>G</span><div><b>GENCOUV</b><small>AI TRADING TECHNOLOGY</small></div></div><p>Portfolio management, trading strategies, automated trading systems, Expert Advisors and market intelligence tools.</p><div><a href="/about">About</a><a href="/copy-trading">Copy Trading</a><a href="/roadmap">Roadmap</a><a href="/marketplace">Trading Bots</a><a href="/testimonials">Client Experiences</a><a href="/login">Log in</a><a href="/signup">Create account</a><a href="#support">Contact support</a><a href="/terms">Terms</a><a href="/privacy">Privacy Policy</a><a href="/risk-disclosure">Risk Disclosure</a><a href={telegram} target="_blank" rel="noreferrer">Talk to Gencouv</a></div><small>Gencouv does not accept or hold client deposits. © 2026 Gencouv. Trading involves substantial risk. Results are not guaranteed and past performance does not guarantee future results.</small></footer>
    <FloatingGencouvChat />
  </main>;
}
