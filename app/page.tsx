...
export default function Home() {
  return <main>
    ...
    <section className="section shell"><div className="sectionHead"><div><div className="eyebrow left">GENCOUV SERVICES</div><h2>One managed portfolio. Additional tools through Trading Bots.</h2></div><p>Portfolio management is our primary client service. Trading Bots separately provides Expert Advisors, indicators and trading utilities as those products become available.</p></div><div className="serviceGrid">{services.map((service,i)=><a className="service" href={i === 0 ? "/portfolio-management" : "/marketplace"} key={service.name}><span>0{i+1}</span><small>{service.label}</small><h3>{service.name}</h3><p>{service.description}</p><b>{i === 0 ? "Learn about portfolio management ↗" : "View Trading Bots ↗"}</b></a>)}</div></section>
