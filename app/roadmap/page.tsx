...
const roadmap = [
  ...
  ["Complete the onboarding evaluation", "Continue with Gencouv on Telegram. The evaluation checks suitability, eligibility and jurisdiction before participation is considered. Passing is not automatic."],
...
export default function RoadmapPage() {
  return <main>
    <nav className="nav shell">
      ...
      <a className="navCta" href={telegram} target="_blank" rel="noreferrer">Get on Board</a>
    </nav>
    ...
    <section id="roadmap" className="section shell">
      ...
      <div className="steps">{roadmap.map(([title,description],i)=><article key={title}><span>{String(i+1).padStart(2,"0")}</span><div><h3>{title}</h3><p>{description}</p>{i===1&&<p style={{marginTop:"10px"}}><a href="/testimonials">Review client experiences ↗</a></p>}{i===2&&<p style={{marginTop:"10px"}}><a href={myfxbook} target="_blank" rel="noreferrer">Open the verified Myfxbook record ↗</a></p>}{i===3&&<p style={{marginTop:"10px"}}><a href={telegram} target="_blank" rel="noreferrer">Continue to evaluation ↗</a></p>}</div></article>)}</div>
    </section>

    <section className="band"><div className="shell split"><div><div className="eyebrow left">ACCOUNT CONTROL</div><h2>Your account remains yours.</h2><p className="lead">The distinction matters: Gencouv manages a strategy through supported infrastructure. It does not operate as the place where clients send or store their deposits.</p></div><div className="steps">{controls.map(([title,description],i)=><article key={title}><span>0{i+1}</span><div><h3>{title}</h3><p>{description}</p></article>)}</div></div></section>
