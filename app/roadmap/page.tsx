...
export default function RoadmapPage() {
  return <main>
    ...
    <section id="roadmap" className="section shell">
      ...
      <div className="steps">{roadmap.map(([title,description],i)=><article key={title}><span>{String(i+1).padStart(2,"0")}</span><div><h3>{title}</h3><p>{description}</p>{i===1&&<p style={{marginTop:"10px"}}><a href="/testimonials">Review client experiences ↗</a></p>}{i===2&&<p style={{marginTop:"10px"}}><a href={myfxbook} target="_blank" rel="noreferrer">Open the verified Myfxbook record ↗</a></p>}{i===3&&<p style={{marginTop:"10px"}}><a href={telegram} target="_blank" rel="noreferrer">Ask Gencouv a question ↗</a></p>}</div></article>)}</div>
    </section>
    ...
    <section className="finalCta shell"><div className="eyebrow">READY FOR THE NEXT STEP?</div><h2>Start with questions, not a deposit.</h2><p>Use Gencouv support to understand the process first. There is no commitment required to ask about the structure, evidence, risks or onboarding process.</p><div className="copyHeroActions"><a className="primary" href={telegram} target="_blank" rel="noreferrer">Talk to Gencouv <span>↗</span></a><a className="secondary" href={myfxbook} target="_blank" rel="noreferrer">Verify historical performance</a></div></section>
